...
100](https://letsencrypt.org/docs/rate-limits/). A wildcard certificate
can be used to avoid that limit.
...
pointing to `$datadir/letsencrypt/acme-challenge` of your odoo instance.
...
`Retry-After` header under which we can instruct the server to retry the
challenge. At the time these lines were written, Boulder had not
...
answer to our challenge, specifically it is the `deadline` parameter of
`poll_and_finalize`.
