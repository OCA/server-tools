...
class RestrictFieldAccessMixin(models.AbstractModel):
    ...
    def _compute_restrict_field_access(self):
        ...
        for this in self:
            this["restrict_field_access"] = any(
                not this._restrict_field_access_is_field_accessible(field, "write")
                for field in self._fields
            )
            if this["restrict_field_access"]:
                result["warning"] = {
                    "title": _("Warning"),
                    "message": _("You will lose access to fields if you save now!"),
                }
        return result
    ...
    def create(self, vals):
        restricted_vals = self._restrict_field_access_filter_vals(vals, action="create")
        return self.browse(
            super(
    ...
    def copy(self, default=None):
        ...
        return self.browse(
            super(RestrictFieldAccessMixin, self._restrict_field_access_suspend())
            ...
            for field in record:
                if not this._restrict_field_access_is_field_accessible(field):
                    ...
                    if self._fields[field] in self.env.cache:
                        self.env.cache[self._fields[field]].pop(record["id"], False)
        return result
    ...
    def read_group(
        ...
        has_inaccessible_field |= any(
            expression.is_leaf(term)
            and not self._restrict_field_access_is_field_accessible(
                term[0].split(".")[0]
            )
            for term in domain
        )
        if groupby:
            if isinstance(groupby, basestring):
                groupby = [groupby]
            has_inaccessible_field |= any(
                not self._restrict_field_access_is_field_accessible(f.split(":")[0])
                for f in groupby
            )
        if orderby:
            has_inaccessible_field |= any(
    ...
    def _BaseModel__export_rows(self, fields):
        ...
        result = []
        for this in self:
            rows = super(RestrictFieldAccessMixin, this)._BaseModel__export_rows(fields)
            for row in rows:
    ...
    def write(self, vals):
        for this in self:
            ...
            vals = this._restrict_field_access_filter_vals(vals, action="write")
    ...
    def _search(
        ...
        for term in args:
            if not expression.is_leaf(term):
                continue
            if not self._restrict_field_access_is_field_accessible(term[0], "read"):
                has_inaccessible_field = True
                break
        if has_inaccessible_field:
            check_self = self if not access_rights_uid else self.sudo(access_rights_uid)
            check_self._restrict_field_access_inject_restrict_field_access_domain(args)
        return super()._search(
            args,
            offset=offset,
            limit=limit,
    ...
    @api.model
    def _restrict_field_access_inject_restrict_field_access_arch(self, arch, fields):
        ...
        if "restrict_field_access" not in fields:
            ...
            fields["restrict_field_access"] = self._fields[
                "restrict_field_access"
            ].get_description(self.env)

    @api.model
    def _restrict_field_access_adjust_field_modifiers(self, field_node, modifiers):
        ...
        if not self._restrict_field_access_is_field_accessible(
            ...
            for modifier, value in [("readonly", True), ("required", False)]:
                ...
                if domain:
                    restrict_domain = expression.OR([restrict_domain, domain])
    ...
    def _restrict_field_access_filter_vals(self, vals, action="read"):
        ...
        return dict(
            filter(
                lambda itemtuple: this._restrict_field_access_is_field_accessible(
