...
class VacuumRule(models.Model):
    ...
    retention_time = fields.Integer(
        ...
        help="Number of days the messages concerned by this rule will be "
             "kept in the database after creation. Once the delay is "
    ...
    def _search_autovacuum_records(self, limit):
        self.ensure_one()
        model = self.ttype
        if model == 'message':
            model = 'mail.message'
            ...
            model = 'ir.attachment'
        return self.env[model]._get_autovacuum_records(self, limit)
