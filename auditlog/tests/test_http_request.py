...
class TestAuditlogHTTPRequest(common.TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        self.request_model = self.env["auditlog.http.request"]
        self.session_model = self.env["auditlog.http.session"]
    ...
    def test_new_http_request_created(self):
        ...
        created = self.request_model.browse(result)
        self.assertTrue(
