...
from os.path import basename, exists, isfile, join, normpath, realpath
...
class IrFilesystemDirectory(models.Model):
    ...
    file_ids = fields.One2many(
        "ir.filesystem.file", compute="_compute_file_ids", string="Files"
    )
    ...
    def _get_directory_files(self):
        ...
        if self.get_dir() and exists(self.get_dir()):
            try:
                ...
                _logger.info(
                    "_get_directory_files reading %s", self.get_dir(), exc_info=True
                )
        return files
