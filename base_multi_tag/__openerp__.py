...
2. Use inherit from ``"res.tag.mixin"`` to get tags functionality
   to your model, like::
...
4. Now You can use ``tag_ids`` field in Your views for Your model:
   ...
   - search view::
        ...
        <field name="tag_ids" string="Tag"
            filter_domain="['|',('tag_ids.name','ilike',self),
                                ('tag_ids.code','ilike',self)]"/>
        <!-- For invers searching (items that do not contain tag)-->
   ...
   - form view::
    ...
    Pay attention on context field. This will allow to avoid tag form popup
    when adding tag from form field


Bug Tracker
===========

Bugs are tracked on `GitHub Issues <https://github.com/OCA/{project_repo}/\
issues>`_.
In case of trouble, please check there if your issue
has already been reported. If you spotted it first, help us smashing it
by providing a detailed and welcomed feedback `here <https://github.com/OCA\
/server-tools/issues/new?body=module:%20base_multi_tag%0Aversion:%20{version}%0A\
%0A**Steps%20to%20reproduce**%0A-%20...%0A%0A**Current%20behavior**%0A\
