...
class InstanceNode(object):
    ...
    @property
    def depends(self):
        # Returns model nodes for each of the fields M2O.
        try:
...
def bail(func, default=None):
    ...
    def inner(node):
        if node in stack:
            print('Ignoring cycle detected: %r' % [n for n in stack + [node]])
            return default
        else:
            stack.append(node)
            try:
...
MAGIC_COLUMNS += ['message_ids', 'rml_footer', 'rml_header',
        ...
        for record in records:
            if record._name not in EXCLUDE_MODELS:
                for name, field in record._fields.items():
                    if name not in MAGIC_COLUMNS and field.store \
...
def find_depends(records):
    # Returns record depends for each of the fields.
    from xoutil.functools import partial, lru_cache
    bail_with_zero = partial(bail, default=0)
    res = get_related(records, 0)
