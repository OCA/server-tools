...
export class CustomInfoRenderer extends Component {
    setup() {
        this.state = useState({
            closedGroupIds: [],
        });
    ...
    getFieldInfo(widget) {
        return {
            FieldComponent: registry.category("fields").get(widget),
            FiledType: "custom",
        };
    }
    getFieldName(type) {
...
CustomInfoRenderer.template = "base_custom_info.table";
CustomInfoRenderer.components = {
    Widget,
    Field,
};
