    ...
    def run(self):
        ...
        for act in actions:
            ...
            if force_sql_debug:
                ...
                # ``odoo.sql_db.Cursor.__init__`` snapshots the logger level once:
                #   ``self.sql_log = _logger.isEnabledFor(logging.DEBUG)``
                # The cursor is created at the start of the transaction.
                # That is long before this method raises the logger level.
                # So ``sql_log`` is already False on the live cursor.
                # ``Cursor.execute`` then gates every debug call on that flag:
                #   ``if self.sql_log: _logger.debug("[%.3f ms] query: %s", ...)``
                # Raising the logger level alone does not flip ``sql_log``.
                # We must set it directly on the cursor to enable query logging.
                act.env.cr.sql_log = True
