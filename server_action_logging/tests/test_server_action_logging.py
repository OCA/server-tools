...
class TestServerActionLogging(TransactionCase):
    ...
    def test_01_log_action_sql_db(self):
        ...
        self.assertTrue(self.log_action.isEnabledFor(logging.INFO))
        self.assertFalse(self.log_sql_db.isEnabledFor(logging.DEBUG))
        # The cursor is created at transaction start, before DEBUG is enabled.
        # ``Cursor.__init__`` therefore sets ``sql_log = False`` on it.
        # The fix must override that flag directly on the live cursor.
        self.assertFalse(self.env.cr.sql_log)
        ...
        self.assertFalse(self.log_sql_db.isEnabledFor(logging.DEBUG))
        # ``cr.sql_log`` must be restored to False after the action completes.
        self.assertFalse(self.env.cr.sql_log)
