...
def _extend_name_search_lang(self, lang, name, args, operator, result, limit):
    res_lang = list(self.with_context(lang=lang)._name_search(
        name, args, operator, limit=limit
    ))
    res_ids = result
    new_res = list(filter(lambda x: x not in res_ids, res_lang))
...
class IrModel(models.Model):
    ...
    name_search_multi_lang = fields.Boolean(
        string="Search Translated Name",default="True",
