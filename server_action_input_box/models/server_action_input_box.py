...
class ServerActionInputBox(models.Model):
    ...
    active_action = fields.Boolean("Active", default=True)
    ...
    apply_to_linked_field_lines = fields.Boolean(
        string="Apply to linked field lines", default=False
    )
...
server_action_input_box_id = env['server.action.input.box'].browse({self.id})
    ...
    def write(self, vals):
        ...
        for rec in self:
            if "ir_action_server_id" not in vals:
                if not rec.ir_action_server_id:
                    rec.ir_action_server_id = rec._create_ir_action_server()
                else:
                    # triggers autocorrect logic
                    rec.ir_action_server_id.write({})
        return res
