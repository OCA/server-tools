    ...
    def button_analyse_code(self):
        ...
        for module in self:
            ...
            for file_path, file_ext in file_list:
                file_res = subprocess.check_output(
                     ["cloc", "--quiet", "--yaml",
                      "--by-file-by-lang", file_path]
                )  # --json only available on recent version
                parsed = yaml.load(file_res)
                for k, v in analysed_datas.get(file_ext).items():
                    try:
                        v['value'] += parsed[file_path].get('code', 0)
                    except Exception as exc:
                        _logger.warn(
                            'File {} can not be analysed: {}'.format(
                                file_path,
                                exc
                            )
                        )
