...
class TestQuickUpgrade(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.module_a = cls.Module.create(
            {"name": "test_qu_a", "state": "installed"}
        )
        cls.module_b = cls.Module.create(
            {"name": "test_qu_b", "state": "installed"}
        )
        # test_qu_b depends on test_qu_a (reverse dependency of A).
        cls.Dependency.create(
            {"name": "test_qu_a", "module_id": cls.module_b.id}
        )

        # An uninstalled upstream dependency of the target module.
        cls.module_c = cls.Module.create(
            {"name": "test_qu_c", "state": "uninstalled"}
        )
        cls.Dependency.create(
            {"name": "test_qu_c", "module_id": cls.module_a.id}
        )

    def setUp(self):
        super().setUp()
        model_cls = type(self.Module)
        patch_update = patch.object(
            model_cls, "update_list", lambda self: None
        )
    ...
    def test_quick_upgrade_requires_installed_module(self):
        """Quick-upgrading a non-installed module raises a UserError."""
        module_d = self.Module.create(
            {"name": "test_qu_d", "state": "uninstalled"}
        )
    ...
    def test_quick_upgrade_empty_recordset_is_noop(self):
        ...
        self.assertFalse(self.Module.browse().button_quick_upgrade())
