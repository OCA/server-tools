...
class BaseSubstateType(models.Model):
    ...
    target_state_field = fields.Char(required=True,
...
class TargetStateValue(models.Model):
    ...
    # Data in this object should be created by import as technical data
    # in specific module ex : sale_subsatate
    name = fields.Char('Target state Name', required=True, translate=True,
            help='Target state translateble name.'
                 ' Ex fore sale order "Quotation", "Sale order", "Locked"...')
    base_substate_type_id = fields.Many2one('base.substate.type',
           string='Substate Type', ondelete='restrict',
        )
    target_state_value = fields.Char(required=True, 
...
class BaseSubstate(models.Model):
    ...
    target_state_value_id = fields.Many2one('target.state.value',
            string='Target State Value', ondelete='restrict')
