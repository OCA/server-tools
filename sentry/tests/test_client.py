...
class TestClientSetup(TransactionCase):
    ...
    @patch("odoo.addons.oware_sentry.hooks.get_git_tag", return_value=GIT_SHA)
    def test_config_odoo_dir(self, get_git_tag):
    ...
    @patch("odoo.addons.oware_sentry.hooks.get_git_tag", return_value=GIT_SHA)
    def test_config_release(self, get_git_tag):
        config.options["sentry_odoo_dir"] = "/opt/odoo/core"
