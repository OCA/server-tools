...
(function () {
    ...
    // Backend bundles ship Odoo's error service, which already routes every
    // window error and rejection to owl_error_boundary.esm.js; the SDK's own
    // global handlers and callback wrappers would report each one twice.
    const SDK_CAPTURE_INTEGRATIONS = ["GlobalHandlers", "BrowserApiErrors", "TryCatch"];
    function hasOdooErrorService() {
    ...
    function buildInitOptions(Sentry, conf, replayOn) {
        const extra = buildIntegrations(Sentry, conf, replayOn);
        const opts = {
            dsn: conf.dsn,
            release: conf.release || undefined,
            environment: conf.environment || undefined,
            integrations: hasOdooErrorService()
                ? (defaults) =>
                      defaults
                          .filter((i) => !SDK_CAPTURE_INTEGRATIONS.includes(i.name))
                          .concat(extra)
                : extra,
        };
    ...
    // Sentry drops tag values longer than 200 characters.
    function tagValue(items) {
        let kept = items;
        let value = kept.join(",");
        while (value.length > 200 && kept.length > 1) {
            kept = kept.slice(0, -1);
            value = `${kept.join(",")},…`;
        }
    ...
    function attachUserContext(Sentry, conf) {
        ...
        if (Array.isArray(conf.categories) && conf.categories.length) {
            Sentry.setTag("odoo.category", tagValue(conf.categories));
        }
    }
