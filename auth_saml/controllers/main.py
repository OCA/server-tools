...
class AuthSAMLController(http.Controller):
    ...
    def signin(self, req, **kw):
        ...
        to us... we need to validate it, disable csrf as user can arrived
        from the IDP directly.
        """
        saml_response = kw.get('SAMLResponse', None)
        relay_state = kw.get('RelayState', None)
        state = {}
        if isinstance(relay_state, (str, unicode)):
            # Okta.com return a weird list of dict wich is not json standard
            relay_state = "[%s]" % relay_state
            relay_state = simplejson.loads(relay_state)
        ...
        if isinstance(relay_state, dict):
            state = relay_state
        provider = state.get('p', None)

        if not relay_state or not provider:
        ...
        with request.registry.cursor() as cr:
            try:
                ...
                credentials = u.auth_saml(
                    cr, SUPERUSER_ID, long(provider), saml_response
                )
                cr.commit()
